document.addEventListener("DOMContentLoaded", () => {
    function submitFormContact() {
        const submitInputCF7 = document.querySelector(".wpcf7-form-control.wpcf7-submit.has-spinner");
        ...
        if (customSubmit && submitInputCF7 && contactForm) {
            ...
            contactForm.addEventListener('wpcf7mailsent', handleFormEvent, false);
            contactForm.addEventListener('wpcf7mailfailed', handleFormEvent, false);
            contactForm.addEventListener('wpcf7invalid', handleFormEvent, false);
    ...
    class CustomSelectBox {
        ...
        init() {
            this.bindEvents();
				...
				let initialSelectedOption = this.selectDropdown.querySelector('.select-option.selected');

				// Nếu không có option nào được đánh dấu là 'selected', chọn option đầu tiên
				if (!initialSelectedOption) {
					initialSelectedOption = this.selectDropdown.querySelector('.select-option');
				}
        ...
        selectOption(option) {
            this.selectDropdown.querySelectorAll('.select-option').forEach(opt => {
            ...
            const cf7CourseSelect = document.querySelector('input[name="your-course"]');
    ...
    const customSelectTemplate = document.getElementById('customSelectTemplate');
    const inpService = document.querySelector('.inp__service');

    if (customSelectTemplate && inpService && typeof courseData !== 'undefined') {
        const customSelectClone = customSelectTemplate.content.cloneNode(true);
        const selectDropdown = customSelectClone.querySelector('#selectDropdown');
        ...
        inpService.insertAdjacentElement('beforeend', customSelectClone.firstElementChild);
