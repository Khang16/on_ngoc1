...
$post_type = 'try'; // Sử dụng post type 'try' riêng biệt
...
// Thực thi query lấy danh sách bài viết
...
<section id="grid-items" data-category="<?= $term_slug; ?>" class="grid-items">
  <div class="grid-items__container">
    <div class="grid-items__header" style="margin-top: 10rem;">
      <h2 class="grid-items__title">Danh sách bài viết Try</h2>
      <div class="grid-items__searchbar">
        ...
        <div class="grid-items__search-container">
          ...
          <input type="text" value="<?= $params['search'] ?? ""; ?>" class="grid-items__search"
            placeholder="Tìm kiếm trong Try" />
        </div>
        ...
        <div class="grid-items__tags">
          <?php foreach ($child_terms as $child_term): ?>
            <input hidden type="checkbox" id="<?= $child_term->taxonomy ?>-<?= $child_term->slug ?>"
              name="<?= $child_term->taxonomy ?>" value="<?= $child_term->slug ?>" <?= in_array($child_term->slug, $categories) ? 'checked' : '' ?>>
    ...
    <!-- Grid hiển thị danh sách bài viết đã load -->
    <div data-limit="<?= $params['limit']; ?>" class="grid-items__grid" id="postGrid">
      <?php
      if ($posts->have_posts()) {
        while ($posts->have_posts()) {
          $posts->the_post();
          // Sử dụng template part riêng cho try hoặc dùng chung với extends
          get_template_part('template-parts/try/grid-items/item-loop');
        }
      } else {
    ...
    <!-- Phân trang -->
    <?php
    $show_pagination = $posts->max_num_pages > 1;
    ?>
    <nav <?= !$show_pagination ? 'style="display: none;"' : '' ?> class="pagination">
      <?php
      // Sử dụng hàm paginate tùy chỉnh hoặc WordPress pagination
      if (function_exists('paginate')) {
        ...
        // Fallback pagination nếu hàm paginate không tồn tại
        $items = range(1, $posts->max_num_pages);
        $prev = max(1, $params['page'] - 1);
        $next = min($posts->max_num_pages, $params['page'] + 1);
      }
      ?>
      <div class="pagination__list">
        <?php foreach ($items as $key => $item):
          $active_page = $item == $params['page'] || ($params['page'] < 2 && $key === 0);
          ?>
...
<?php endif; ?>
<!-- Template card - được dùng để clone bằng JS -->
